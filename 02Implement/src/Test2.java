...
public class Test2 {
	 ...
	 * 입력조건 : 세로 N,가로 M(3~50)
	 * 
	 * 칸의 좌표(A,B)와 방향 d 가 공백으로 구분 d의 값 0 : 북쪽 1 : 동쪽 2 : 남쪽 3 : 서쪽 셋째 줄 부터 맵이 육지인지
	 * 바다인지에 대한 정보가 주어짐. N개의 줄에 맵의 상태가 북쪽부터 남쪽 순서대로, 각 줄의 데이터는 서쪽부터 동쪽 순서대로 주어짐. 맵의
	 * 외곽은 항상 바다 0 : 육지 1 : 바다
	 * 
	...
	// 북,동,남,서 방향 정의 -> 행렬로 접근하기 때문
	public static int dy[] = { 0, 1, 0, -1 };
	public static int dx[] = { -1, 0, 1, 0 };

	// 왼쪽으로 회전
	public static void turnLeft() {
		// 처음 direction은 0(북쪽)으로 들어오고 서쪽으로 회전해야 하기때문에 -1을 해주고 direction을 서쪽으로 바꿔줌.
		direction -= 1;
		if (direction == -1)
			direction = 3;
	}
	...
	public static void main(String[] args) {
		...
		d[x][y] = 1;// 현재 좌표 방문 처리 방문 : 1, 방문x : 0
